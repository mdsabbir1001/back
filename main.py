...
async def get_portfolio_projects(category_name: Optional[str] = None):
    try:
        query = supabase.table('portfolio_projects').select("*, portfolio_categories(name)")
        if category_name:
...
async def create_package(package: Package, user: dict = Depends(get_current_user)):
    try:
        response = supabase.table('packages').insert(package.dict()).execute()
        return response.data
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def update_package(package_id: str, package: Package, user: dict = Depends(get_current_user)):
    try:
        response = supabase.table('packages').update(package.dict()).eq("id", package_id).execute()
        return response.data
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
